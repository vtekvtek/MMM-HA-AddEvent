...
Module.register("MMM-HA-AddEvent", {
  ...
  _buildOnce() {
    ...
    // prevent "invisible click zones" from closing when tapping inside modal
    ...
    summary.addEventListener("focus", () => this._setActiveField("ha_summary"));
    ...
    desc.addEventListener("focus", () => this._setActiveField("ha_desc"));
  ...
  _setActiveField(id) {
    ...
    if (this._activeField === id) {
      // Still ensure keyboard input is synced
      this._syncKeyboardToActive();
      return;
    }
  ...
  _initKeyboardIfNeeded() {
    ...
    // Extra insurance for class
    kbEl.classList.add("haKbTheme");

    // Ensure caps visual matches current state
    this._applyCapsVisual();
  },
  ...
  _syncKeyboardToActive() {
    ...
    // If field already has text, don't auto-cap
    if (v.length > 0 && this._autoCapNext) {
  ...
  _onKbChange(input) {
    ...
    if (this._autoCapNext && before.length === 0 && input.length >= 1) {
      this._autoCapNext = false;
      this._applyKeyboardCaseMode(true);
    }
  ...
  _onKbKeyPress(btn) {
    ...
    // Normal key: if shift is active, reset after it lands
  ...
  _submit() {
    ...
    if (this._current.allDay) {
      ...
      // Home Assistant calendar.create_event all-day is date based, end_date is exclusive
